...
export class ArticleDetailComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.dataService.articles$.subscribe(articles => {
      const found = articles.find(a => String(a.id) === String(this.id));
      if (found) {
        this.article = { ...this.article, ...found };
        const idx = articles.findIndex(a => String(a.id) === String(this.id));
        this.prevArticle = idx > 0 ? articles[idx - 1] : null;
        this.nextArticle = idx < articles.length - 1 ? articles[idx + 1] : null;
        this.relatedArticles = articles
          .filter(a => String(a.id) !== String(this.id))
          .slice(0, 3);
      } else {
        this.notFound = true;
      }
      const raw = this.article.content || this.article.excerpt || '';
      this.contentParagraphs = raw.split('\n\n').map((p: string) => p.trim()).filter((p: string) => p.length > 0);
    }).unsubscribe();
  }
